...
export default function FrontHeader() {
  ...
  const HandleLogout = () => {
    ...
    axios.get(`${process.env.REACT_APP_BASE_URL}/${type}/auth/logout`, {
      ...
      if(type === 'client') {
          const client_id = getCidFromStorageToken();
          const data_update = {
      ...
      debugger;
      // change online / offline and notify that advisor changed the online status to all ()
      // in my case, if there are several logined advisor as one advisor, then one of them log out , should notify all advisors
      if(type === 'advisor') {
        ...
        socket.emit('ADVISOR-ONLINE-OFFLINE', ({advisor:adv.id , status: 0}));
      ...
      <header className="bg-white frontend-header fixed w-full top-0" style={{ boxShadow: 'rgb(0 0 0 / 9%) 0px 9px 17px -4px' }}>
        ...
        <div className="container">
          <div className="row flex-nowrap justify-content-between align-items-center">
            ...
            <div className="col-5 col-md-3 ps-3 mlogo">
              <NavLink to={ url } className="navbar-brand fsp-25 block ffq">
                {
                  logo &&
                  // <img src={`${process.env.REACT_APP_BASE_URL}/${logo}`} alt='logo' />
                  // <img src='/logo1.svg' alt='logo' />
                  <img src='/idea1.svg' alt='logo' />
                }
                
              </NavLink>
            </div>
            <div className="col-7 col-md-7 pb-0 px-0 mx-0">
              {name !== '' &&
              ...
              {isLoggedIn == 'false' &&
                <>
                  <div className='mlogin float-right'>
                    <Link to="/client-login" className="btn btn-white me-lg-2 d-lg-inline fw-700">Log In</Link>
                    <Link to="/client-signup" className="btn btn-primary rounded-1">Sign Up</Link>
                  </div>
                </>
              }
