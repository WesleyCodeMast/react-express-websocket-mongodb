...
export default function ClientSignupStep() {
    ...
    const HandleSignup = () => {
        ...
        formData.append("category", '649a7993a1008dcd3ad0fd80');
        formData.append("status", 0);

        if(checkTerms != '') {

            if(localStorage.getItem('guest_advisor_id')) {
                axios.patch(`${process.env.REACT_APP_BASE_URL}/frontend/advisor/${localStorage.getItem('guest_advisor_id')}`, formData, {
                    headers: {
                    ...
                    if (result.data.success === true) {
                        ...
                        if (result.data.error) {
                            ...
                            swal("error", result.data.data, "error");
                        }
                    }
                }).catch(err => {
                    if (err.response.data.error) {
                        ...
                        swal("error", "This email address is already registered", "error");
                    }
                })
            } else {
                axios.post(`${process.env.REACT_APP_BASE_URL}/advisor/auth/register`, formData, {
                    ...
                    if (result.data.status === 200) {
                        localStorage.setItem('guest_advisor_id',result.data.id);
                        setActiveStep(4);
                    } else if (result.status === 203) {
                        if (result.data.data.errors) {
                            setErrors(result.data.data.errors);
            ...
            swal("error", "Please click on terms and conditions checkbox", "error");
        }
    }
    ...
    const HandleStep2 = () => {
        ...
        } else if(localStorage.getItem('ad_userName')) {
            setActiveStep(2);
        } else {
                const data = {
    ...
    const onChangeAlphaNumericInput = (e) => {
       if (/[^[A-zÀ-Ÿ\d-]*$]/.test(e.target.value)) {
            swal("error","Only alphanumeric value accepted",'error');
            setUserName('');
       } else {
    ...
    const HandleSubmit = () => {
        if (window.confirm("Are you sure to submit details?")) {
            const data = {
               "email": localStorage.getItem('ad_email'),
            }
              ...
              headers: {
                ...
                if(result.status === 201) {
                  swal("Hello", result.data.data, "error");     
                    ...
                    localStorage.removeItem('guest_advisor_id')
                                        ...
                                        {/* <div className='card-body p-0 py-2'>
                                            ...
                                            <div className='px-md-3 pt-2 pb-4'>
                                                ...
                                                <div className='position-absolute bottom-0'>
                                                    <DropzoneField designType={1} onDrop={onDrop} text={userPicture ? 'Change Picture' : 'Browse Picture'} className="btn btn-outline-primary bg-light-dark text-hover-primary mb-n2  px-4 rounded-1" />
                                                </div>
                                                </div>
                                            </div>
                                            <hr />
                                                ...
                                                <div className='mt-3 px-lg-5 clientstep3'>
                                                    <form className='px-lg-5'>
                                                        {/* <div className='form-group px-lg-5'>
                                                            <label className='fw-700'>Upload Proile Picture </label>  
                                                            <div className='uploadimg'>
                                                                <input
                                                                    type="file"
                                                        ...
                                                        <div className='form-group px-lg-5'>
                                                            ...
                                                            <PhoneInput
                                                                  ...
                                                                  value={phone}
                                                                  className='form-control py-2 bg-light-dark border-0 h-12 rounded-1' 
                                                                  onChange={setPhone} 
                                                            />
                                                            { errors && errors.phone && <>
                                                                    <span style={{ color:'red' }}>Phone Number is required</span><br />
                                                                </> }
                        
                                                        </div>
                                                        <div className='form-group mt-4 px-lg-5'>
                                                            <label className='fw-700'>Date of Birth <span className='text-red text-base'>*</span></label>
                                                            <input type="date" name='dob' onChange={(e) => setDob(e.target.value)} max={date} className='form-control h-12 py-2 bg-light-dark border-0 rounded-1' />
                                                            { errors && errors.dob && <>
                                                                    <span style={{ color:'red' }}>Date of birth is required</span><br />
                                                                </> }
                                                        </div>
                                                        <div className='form-group mt-4 px-lg-5'>
                                                            <label className='fw-700'>Address </label>
                                                            <textarea name='address' onChange={(e) => setAddress(e.target.value)} className="form-control text-sm mb-4 bg-light-dark" cols={5} rows={5} placeholder="Address" required></textarea>
                                                        </div>
                                                        <div className='form-group mt-4 px-lg-5'>
                                                            ...
                                                            { errors && errors.country && <>
                                                                <span style={{ color:'red' }}>Country is required</span><br />
                                                            </> }
                                                        </div>
                                                        <div className='form-group mt-4 px-lg-5'>
                                                        ...
                                                        <div className='form-group mt-4 px-lg-5'>
                                                            ...
                                                            <input type="text" name='city' onChange={(e) => setZipCode(e.target.value)} className='form-control py-2 bg-light-dark h-12 border-0 rounded-1 mb-4' />
                                                        </div>
                                                        <div className='form-group mt-4 px-lg-5'>
                                                        <label className='fw-700'>Zip Code</label>
                                                            <input type="text" name='zipcode' onChange={(e) => setZipCode(e.target.value)} className='form-control py-2 bg-light-dark h-12 border-0 rounded-1' />
                                                        </div>
                                                        <div className='mt-4 px-lg-5'>
                                                            <label className="font-semibold inline-flex">
                                                                ...
                                                                I have read and agree to the Data Collection Policy for Advisors. {" "}
                                                                <a
                                                                    target="_blank"
                                                                    href="/data-collection-policy-advisors"
                                                                    className="text-regal-blue"
    
                                                                >
                                                                    Data Collection Policy for Advisor
                                                                </a>
                                                                </span>
                                                            </label>
                                                        </div>
                                        ...
                                        <div className='card-body p-0 py-2'>
                                            ...
                                            <div className='d-flex justify-content-center align-items-center py-2'>
                                                <div className='d-flex'>
                                                    ...
                                                    <Link onClick={HandleSubmit} className='group relative flex justify-center rounded-1 bg-purple-900 py-2 px-4 text-sm text-white hover:btn-light'>Submit</Link>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                                    ...
                                                    <div className='col-12 col-md-6 text-center'>
                                                        ...
                                                        <div className='mt-3'>
                                                            <div className='rounded-3 py-4' style={{ border: '2px dashed #f1f1f1' }}>
                                                                ...
                                                                <button className='btn border-0 bg-light-dark text-primary'>Upload file</button>
                                                            </div>
                                                        </div>
                                                    </div>
