...
module.exports = {
  getAllPayments: async (req, res, next) => {
    ...
    const filter_status = req.query.filter_status
      ...
      console.log('advisor',req.user._id);

      var totalPosts = await Payment.countDocuments({advisor: req.user._id, ...filter_created_at, ...filter_status, ...filter_advisor, ...filter_client});
      ...
      const totals = await Payment.aggregate([
          ...
          { "$group": { _id: { advisor: "$advisor" }, "totlAmt": { "$sum": "$amount" }, "totlCom": { "$sum": "$app_fee" } } }
        ]);

      console.log("this is advisor payment controller...") 

      // let totalsAm = totals[0].totlAmt;
      // let commis = totals[0].totlCom;
      let totalsAm = 10000;
      let commis = 100;
      let earning = totalsAm-commis;

      Payment.find({advisor: req.user._id, ...filter_created_at, ...filter_status, ...filter_advisor, ...filter_client}, {__v: 0, updatedAt: 0, advisor_id: 0, client_id: 0},
