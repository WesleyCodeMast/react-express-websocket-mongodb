...
var adminLogoPath = __dirname + '/uploads/admin/logo/';
...
if (!fs.existsSync(adminLogoPath))
  fs.mkdirSync(adminLogoPath, { recursive: true });
...
const FrontendAdvisorServiceRoute = require('./Routes/Frontend/Service.route');
const FrontendSettingRoute = require('./Routes/Frontend/Setting.route');
...
app.use('/frontend/advisor-services', FrontendAdvisorServiceRoute);
...
app.use('/frontend/settings', FrontendSettingRoute);

const NotificationRoutes = require('./Routes/Notification.route');
const { writeBlobToPNG } = require("./Utils/writeBlobToImage");
const { saveMessage, updateChatStatus } = require("./services/MessageService");
...
io.on("connection", (socket) => {
  ...
  socket.on('JOIN_ROOM', ({ room, user, clientTz, advisorTz }) => {
    console.log("JOIN ROOM", room)
    roomId = room;

    onlineUsers.set(user, socket.id);
    // set timezone both of advisor and client when join room.
    onlineUsers.set("clientTz", clientTz);
    if(advisorTz != null && advisorTz != "")
      onlineUsers.set("advisorTz", advisorTz);
  ...
  socket.on('NEW_MESSAGE', (msg) => {
    // catch timezones of advisor and client and then add it to the msg... 
    msg['clientTz'] = onlineUsers.get('clientTz');
    msg['advisorTz'] = onlineUsers.get('advisorTz');
    if(saveMessage(msg) && updateChatStatus(msg))
  ...
  socket.on("upload", ({ file, name }, callback) => {
    ...
    var thumbnail_dir = __dirname + '/uploads/chat/thumbnails/' + name;
    ...
    sharp(file)
    .resize(200, 200)
    .toFile(thumbnail_dir + '/' + file.name, (err, info) => {
      if (err) {
        console.error(err);
        return res.status(500).send('Error creating thumbnail');
      }
      res.status(200).send('Thumbnail created and saved');
    });
    writeFile(folder, buffer, 'binary', (err) => {
  ...
  socket.on('leave-chat', ({ room , advisor , client , action }) => {
    ...
    setTimeout(() => {
      ...
      io.emit('CLIENT-END-CHAT-NOTIFY', ({ advisor  ,client }));
    },1000);
  });
  
  socket.on('NOTIFY-TO-ADVIOR-TIMERS', ( { advisor , client , startTime, endTime }) => {
    io.emit('NOTIFICATION-SEND-ADVISOR-RECIEPT', ({ advisor , client , startTime, endTime }));
  });
  ...
  socket.on('TERMINATE-CHAT-FOR-INTERNET-DISCONNECT' , ({ room , advisor , client , action }) => {
      io.to(room).emit('TERMINATE-CHAT-NOTIFY', ({ advisor , client , room }));
      io.emit('TERMINATE-CHAT-RECEIPT', ({ advisor , client , room }));
  });

  socket.on('ADVISOR-ONLINE-OFFLINE' , ({ advisor , status }) => {
    io.emit('ADVISOR-ONLINE-OFFLINE-NOTIFYBACK', ({ advisor , status }));
 });
